...
const APP_PAGO_RETURN = 'beautyapp://pedido-pago'; // al volver del checkout
...
/**
 * Webhook de Mercado Pago. Se invoca con ?pedido=<id> (lo seteamos en la
 * notification_url) y el id del pago. Verifica el pago con el token del
 * proveedor y, si está aprobado, marca el pedido como pagado.
 */
exports.mpWebhook = onRequest({ region: REGION }, async (req, res) => {
  try {
    ...
    if (!pedidoId || !paymentId || (topic && topic !== 'payment')) {
      res.sendStatus(200);
      return;
    }
